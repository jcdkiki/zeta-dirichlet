...
public:
	CoefficientsPlotPanel(wxWindow *parent, const int *args)
		: PlotPanel(parent, args)
	{}
	
	void FitPlot();
	void SetCoefficients(const std::vector<double> &coefficients) { this->coefficients = coefficients; }
	virtual ~CoefficientsPlotPanel() = default;
};

class DynamicPlotPanel : public PlotPanel {
public:
	DynamicPlotPanel(wxWindow *parent, const int *args) : PlotPanel(parent, args), func(nullptr), user_ptr(nullptr) {}
